...
// ── Types for dynamic entries ─────────────────────────────────────
interface StatEntry  { num: string; label: string; }
interface ToolEntry  { name: string; ctx: string; image: string; }
interface ExpEntry   { role: string; company: string; tagline: string; period: string; duration: string; isCurrent: boolean; bullets: string; stack: string; }
interface ProjEntry  { title: string; year: string; emoji: string; desc: string; stack: string; impact: string; challenge: string; liveUrl: string; githubUrl: string; }
...
const DEFAULT_EXPS:  ExpEntry[]   = [];
const DEFAULT_PROJS: ProjEntry[]  = [];
const DEFAULT_TESTS: TestEntry[]  = [];
...
export default function DynamicSectionPage() {
  ...
  // MintSlate dynamic widgets
  const [tools, setTools] = useState<ToolEntry[]>(DEFAULT_TOOLS);
  const [exps,  setExps]  = useState<ExpEntry[]>(DEFAULT_EXPS);
  const [projs, setProjs] = useState<ProjEntry[]>(DEFAULT_PROJS);
  const [tests, setTests] = useState<TestEntry[]>(DEFAULT_TESTS);
  ...
  useEffect(() => {
    Promise.all([
      ...
      if (contentRes.success) {
        ...
        setTools(parseJson<ToolEntry>(c.toolsJson, DEFAULT_TOOLS));
        setExps(parseJson<ExpEntry>(c.expJson, DEFAULT_EXPS));
        setProjs(parseJson<ProjEntry>(c.projJson, DEFAULT_PROJS));
        setTests(parseJson<TestEntry>(c.testJson, DEFAULT_TESTS));
      }
  ...
  async function handleSave() {
    ...
    // MintSlate dynamic sections
    if (templateSlug === 'mintslate') {
      if (sectionKey === 'skills')       merged.toolsJson = JSON.stringify(tools);
      if (sectionKey === 'experience')   merged.expJson   = JSON.stringify(exps);
      if (sectionKey === 'projects')     merged.projJson  = JSON.stringify(projs);
  ...
  // For Maren stats widget: hide stat fields from generic card; for mintslate show them
  ...
  return (
    ...
    >
      ...
      {/* ── MintSlate: Tools dynamic widget ── */}
      ...
      {/* ── MintSlate: Experience dynamic widget ── */}
      {templateSlug === 'mintslate' && sectionKey === 'experience' && (
        <Card>
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setExps(e => [...e, { role: '', company: '', tagline: '', period: '', duration: '', isCurrent: false, bullets: '', stack: '' }])} className={addBtn}>
            ...
            {exps.map((exp, i) => (
              <div key={i} className="p-4 rounded-xl border border-gray-200 bg-gray-50 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setExps(e => e.filter((_, j) => j!==i))} className={removeBtn}><X size={14}/></button>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div><Label>Role / Title</Label><input value={exp.role} onChange={e => setExps(es => es.map((x, j) => j===i ? {...x, role: e.target.value} : x))} placeholder="Senior Engineer" className={`${fieldInput} w-full`}/></div>
                  <div><Label>Company</Label><input value={exp.company} onChange={e => setExps(es => es.map((x, j) => j===i ? {...x, company: e.target.value} : x))} placeholder="Acme Inc" className={`${fieldInput} w-full`}/></div>
                  <div><Label>Tagline</Label><input value={exp.tagline} onChange={e => setExps(es => es.map((x, j) => j===i ? {...x, tagline: e.target.value} : x))} placeholder="Team focus / product context" className={`${fieldInput} w-full`}/></div>
                  <div><Label>Period</Label><input value={exp.period} onChange={e => setExps(es => es.map((x, j) => j===i ? {...x, period: e.target.value} : x))} placeholder="2023 – Present" className={`${fieldInput} w-full`}/></div>
                  <div><Label>Duration label</Label><input value={exp.duration} onChange={e => setExps(es => es.map((x, j) => j===i ? {...x, duration: e.target.value} : x))} placeholder="2 yrs" className={`${fieldInput} w-full`}/></div>
                  <div className="flex items-end gap-2 pb-0.5">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" checked={exp.isCurrent} onChange={e => setExps(es => es.map((x, j) => j===i ? {...x, isCurrent: e.target.checked} : x))} className="w-4 h-4 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500"/>
                ...
                <div>
                  ...
                  <textarea value={exp.bullets} onChange={e => setExps(es => es.map((x, j) => j===i ? {...x, bullets: e.target.value} : x))}
                    rows={4} placeholder="Reduced cold start times by **40%**&#10;Built edge middleware for A/B testing"
                    className="w-full px-2.5 py-1.5 rounded-md border border-gray-200 bg-white text-sm text-gray-900 placeholder:text-gray-300 focus:outline-none focus:ring-1 focus:ring-emerald-500 resize-y"/>
                </div>
                <div>
                  <Label>Tech stack (comma-separated)</Label>
                  <input value={exp.stack} onChange={e => setExps(es => es.map((x, j) => j===i ? {...x, stack: e.target.value} : x))}
                    placeholder="TypeScript, React, Node.js" className={`${fieldInput} w-full`}/>
                </div>
              </div>
            ))}
            {exps.length === 0 && <p className="text-xs text-gray-400 text-center py-4">No experience entries yet. Click &quot;Add Role&quot; to get started.</p>}
          </div>
        </Card>
      )}

      {/* ── MintSlate: Projects dynamic widget ── */}
      {templateSlug === 'mintslate' && sectionKey === 'projects' && (
        <Card>
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setProjs(p => [...p, { title: '', year: '', emoji: '🚀', desc: '', stack: '', impact: '', challenge: '', liveUrl: '', githubUrl: '' }])} className={addBtn}>
            ...
            {projs.map((proj, i) => (
              <div key={i} className="p-4 rounded-xl border border-gray-200 bg-gray-50 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <button onClick={() => setProjs(p => p.filter((_, j) => j!==i))} className={removeBtn}><X size={14}/></button>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  <div className="col-span-2"><Label>Title</Label><input value={proj.title} onChange={e => setProjs(ps => ps.map((x, j) => j===i ? {...x, title: e.target.value} : x))} placeholder="Project name" className={`${fieldInput} w-full`}/></div>
                  <div><Label>Year</Label><input value={proj.year} onChange={e => setProjs(ps => ps.map((x, j) => j===i ? {...x, year: e.target.value} : x))} placeholder="2024" className={`${fieldInput} w-full`}/></div>
                  <div><Label>Emoji</Label><input value={proj.emoji} onChange={e => setProjs(ps => ps.map((x, j) => j===i ? {...x, emoji: e.target.value} : x))} placeholder="🚀" className={`${fieldInput} w-full`}/></div>
                  <div><Label>Impact</Label><input value={proj.impact} onChange={e => setProjs(ps => ps.map((x, j) => j===i ? {...x, impact: e.target.value} : x))} placeholder="10k users" className={`${fieldInput} w-full`}/></div>
                  <div><Label>Challenge solved</Label><input value={proj.challenge} onChange={e => setProjs(ps => ps.map((x, j) => j===i ? {...x, challenge: e.target.value} : x))} placeholder="Real-time at scale" className={`${fieldInput} w-full`}/></div>
                </div>
                <div>
                  <Label>Description</Label>
                  <textarea value={proj.desc} onChange={e => setProjs(ps => ps.map((x, j) => j===i ? {...x, desc: e.target.value} : x))}
                    rows={2} placeholder="Short description of what this project does and why it matters."
                    className="w-full px-2.5 py-1.5 rounded-md border border-gray-200 bg-white text-sm text-gray-900 placeholder:text-gray-300 focus:outline-none focus:ring-1 focus:ring-emerald-500 resize-y"/>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div><Label>Tech stack (comma-sep)</Label><input value={proj.stack} onChange={e => setProjs(ps => ps.map((x, j) => j===i ? {...x, stack: e.target.value} : x))} placeholder="React, TypeScript" className={`${fieldInput} w-full`}/></div>
                  <div></div>
                  <div><Label>Live URL</Label><input type="url" value={proj.liveUrl} onChange={e => setProjs(ps => ps.map((x, j) => j===i ? {...x, liveUrl: e.target.value} : x))} placeholder="https://..." className={`${fieldInput} w-full`}/></div>
                  <div><Label>GitHub URL</Label><input type="url" value={proj.githubUrl} onChange={e => setProjs(ps => ps.map((x, j) => j===i ? {...x, githubUrl: e.target.value} : x))} placeholder="https://github.com/..." className={`${fieldInput} w-full`}/></div>
                </div>
              </div>
            ))}
            {projs.length === 0 && <p className="text-xs text-gray-400 text-center py-4">No projects yet. Click &quot;Add Project&quot; to get started.</p>}
          </div>
        </Card>
      )}

      {/* ── MintSlate: Testimonials dynamic widget ── */}
              ...
              <div key={i} className="p-4 rounded-xl border border-gray-200 bg-gray-50 space-y-3">
                ...
                <div>
                  ...
                  <textarea value={t.quote} onChange={e => setTests(ts => ts.map((x, j) => j===i ? {...x, quote: e.target.value} : x))}
                    rows={3} placeholder="What they said about working with you…"
                    className="w-full px-2.5 py-1.5 rounded-md border border-gray-200 bg-white text-sm text-gray-900 placeholder:text-gray-300 focus:outline-none focus:ring-1 focus:ring-emerald-500 resize-y"/>
                </div>
                <div className="grid grid-cols-3 gap-2">
      ...
      {/* ── Generic fields from template config ── */}
      <Card>
        <div className="space-y-4">
          {fields.map(field => {
            if (field.type === 'color') {
              ...
              return (
                <div key={field.key}>
                  ...
                  <div className="flex items-center gap-3 flex-wrap">
                    {isPhoto && c(field.key) && (
                      <img src={c(field.key)} alt="Profile" className="w-14 h-14 rounded-full object-cover border border-gray-200"/>
                    )}
